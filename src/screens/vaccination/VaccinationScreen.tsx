...
type TabDef = {
  key: VaccineFilterType
  label: string
  statusKey?: 'done' | 'upcoming' | 'overdue'
}
...
export const VaccinationScreen: React.FC = () => {
  ...
  const daysRelative = useCallback(
    (iso: string): string => {
      ...
      if (diff > 0)
        return diff === 1
          ? t('vaccination.relInOneDay')
          : t('vaccination.relInDays', { n: diff })
      const abs = Math.abs(diff)
      return abs === 1
        ? t('vaccination.relOneDayAgo')
        : t('vaccination.relDaysAgo', { n: abs })
    },
    [t]
  )
  ...
  const statusCfg = useMemo(
    () => ({
      ...
      upcoming: {
        ...STATUS_BASE.upcoming,
        label: t('vaccination.statusUpcoming')
      },
  ...
  const TABS: TabDef[] = useMemo(
    () => [
      ...
      {
        key: 'upcoming',
        label: t('vaccination.tabUpcoming'),
        statusKey: 'upcoming'
      },
      {
        key: 'overdue',
        label: t('vaccination.tabOverdue'),
        statusKey: 'overdue'
      },
  ...
  const fetchList = useCallback(
    async (type: VaccineFilterType, silent = false) => {
      ...
      try {
        ...
        data.sort((a, b) => {
          const order = { overdue: 0, upcoming: 1, done: 2 } as Record<
            string,
            number
          >
  ...
  const handleToggle = async (item: VaccinationRecord) => {
    ...
    try {
          ...
          ? {
              ...
              status: newValue
                ...
                : ((new Date(r.scheduledDate) < new Date()
                    ? 'overdue'
                    : 'upcoming') as VaccinationRecord['status'])
            }
          : r
      ...
      Alert.alert(
        t('vaccination.errorTitle'),
        err.message ?? t('vaccination.updateFailed')
      )
    } finally {
      setToggling((prev) => {
  ...
  const renderCard = ({ item }: { item: VaccinationRecord }) => {
    const cfg =
      statusCfg[item.status as keyof typeof statusCfg] ?? statusCfg.upcoming
    const typeClr =
      VACCINE_TYPE_COLOR[item.vaccine?.vaccineType] ?? Colors.textMuted
      ...
      >
        ...
        <View style={styles.cardBody}>
          ...
          {item.vaccine?.description && (
            <Text style={styles.vaccineDesc} numberOfLines={1}>
              {item.vaccine.description}
            </Text>
          )}
          ...
          <View style={styles.cardBottom}>
            ...
            <View style={styles.metaItem}>
              ...
              <Text
                style={[
                  styles.metaText,
                  item.status === 'overdue' && { color: Colors.danger }
                ]}
              >
            ...
            <View style={[styles.typeBadge, { borderColor: typeClr }]}>
              <Text style={[styles.typeText, { color: typeClr }]}>
        ...
        <Switch
          ...
          thumbColor={
            item.status === 'done' ? Colors.success : Colors.textMuted
          }
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
                ...
                <>
                  <View style={styles.progressTop}>
                    <View>
                      <Text style={styles.progressTitle}>
                        {t('vaccination.progressTitle')}
                      </Text>
                      <Text style={styles.progressSub}>
                        {t('vaccination.progressSub', {
                          done: stats.done,
                          total: stats.total
                        })}
                      </Text>
                  ...
                  <View style={styles.statsRow}>
                    <StatChip
                      ...
                      color="#86EFAC"
                      label={t('vaccination.done')}
                      value={stats.done}
                    />
                    <StatChip
                      icon="time-outline"
                      color="#FDE68A"
                      label={t('vaccination.soon')}
                      value={stats.upcoming}
                    />
                    <StatChip
                      icon="alert-circle"
                      color="#FCA5A5"
                      label={t('vaccination.overdue')}
                      value={stats.overdue}
                    />
                  </View>
                </>
          ...
          listLoading ? (
            ...
            <View style={styles.emptyWrap}>
              ...
              <Text style={styles.emptySub}>
                {activeTab === 'done'
                  ? t('vaccination.emptyDone')
                  : activeTab === 'overdue'
                    ? t('vaccination.emptyOverdue')
                    : activeTab === 'upcoming'
      ...
      >
        ...
        {selected &&
          (() => {
            const cfg =
              statusCfg[selected.status as keyof typeof statusCfg] ??
              statusCfg.upcoming
            const typeClr =
              VACCINE_TYPE_COLOR[selected.vaccine?.vaccineType] ??
              Colors.textMuted
            return (
              <View style={styles.modalSheet}>
                ...
                <View
                  style={[styles.modalIconCircle, { backgroundColor: cfg.bg }]}
                >
                ...
                <View style={styles.modalDetails}>
                  <DetailRow
                    ...
                    label={t('vaccination.modalDescription')}
                    value={selected.vaccine?.description ?? '—'}
                  />
                  <DetailRow
                    icon="calendar-outline"
                    label={t('vaccination.modalScheduled')}
                    value={formatDate(selected.scheduledDate)}
                  />
                  <DetailRow
                    icon="time-outline"
                    label={t('vaccination.modalWhen')}
                    value={daysRelative(selected.scheduledDate)}
                    highlight={selected.status === 'overdue'}
                  />
                  <DetailRow
                    icon="medical-outline"
                    label={t('vaccination.modalDoseShort')}
                    value={`${t('babyDetail.dose')} ${selected.vaccine?.dose ?? '—'}`}
                  />
                  <DetailRow
                    icon="flask-outline"
                    label={t('vaccination.modalVaccineType')}
                    value={selected.vaccine?.vaccineType ?? '—'}
                    valueColor={typeClr}
                  />
                  <DetailRow
                    icon="calendar-number-outline"
                    label={t('vaccination.modalAgeRequired')}
                    value={
                      selected.vaccine?.ageRequired === 0
                        ? t('vaccination.atBirth')
                        : selected.vaccine?.ageRequired === 1
                          ? t('vaccination.ageMonthOne', { n: 1 })
                          : t('vaccination.ageMonths', {
                              n: selected.vaccine?.ageRequired ?? 0
                            })
                    }
                  />
                  {selected.vaccine?.isBooster && (
                    <DetailRow
                      ...
                      label={t('vaccination.modalType')}
                      value={t('vaccination.modalBoosterType')}
                    />
                  )}
                  {selected.vaccine?.repeat && (
                    <DetailRow
                      icon="repeat-outline"
                      label={t('vaccination.modalSchedule')}
                      value={t('vaccination.modalRecurring')}
                    />
                  )}
                </View>
