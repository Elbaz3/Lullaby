...
//
//  Flow:
//  1. User enters identifier (email or phone)
//  2. POST /auth/forgot-password → sends OTP
//  3. Navigate to OTP screen with reason="reset"
//  4. OTP screen verifies code
//  5. Navigate to NewPassword with identifier+otp
//  6. NewPassword calls POST /auth/verify-forgot-password
//     with identifier + otp + password + passwordConfirm
// ─────────────────────────────────────────────
...
export const ForgotPasswordScreen: React.FC = () => {
  ...
  const handleSend = async () => {
    if (!validate()) return;
    try {
      // POST /auth/forgot-password — just sends OTP, no token needed
      await forgotPassword(identifier.trim());
      // Navigate to OTP screen, pass mode='forgot' for UI text
